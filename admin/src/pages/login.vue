<template>
  <v-app>
    <AdminHeader />
    <!-- Main Content -->
    <v-main style="background-color: #f0f2f5">
      <v-container fluid class="fill-height d-flex align-center justify-center">
        <v-card width="400" class="pa-4">
          <v-img
            src="https://www2.naga.gov.ph/wp-content/uploads/2022/05/Naga_City_Official_Seal-1.png"
            alt="LGU Seal"
            width="80"
            height="70"
            contain
            class="mx-auto mb-2"
          />

          <div class="text-center">
            <h3 class="font-weight-bold mb-1">Login</h3>
          </div>

          <v-form ref="loginForm" v-model="valid" @submit.prevent="login">
            <v-text-field
              v-model="username"
              label="Username"
              placeholder="Enter your username"
              prepend-inner-icon="mdi-account"
              variant="outlined"
              :rules="[(v) => !!v || 'Username is required']"
            ></v-text-field>

            <v-text-field
              v-model="password"
              label="Password"
              placeholder="Enter your password"
              prepend-inner-icon="mdi-lock"
              :append-inner-icon="show ? 'mdi-eye' : 'mdi-eye-off'"
              :type="show ? 'text' : 'password'"
              @click:append-inner="show = !show"
              variant="outlined"
              :rules="[(v) => !!v || 'Password is required']"
            ></v-text-field>

            <v-alert v-if="error" type="error" class="my-2" dense>
              {{ error }}
            </v-alert>

            <v-select
              v-model="role"
              :items="roles"
              label="Select Role"
              prepend-inner-icon="mdi-briefcase-account"
              variant="outlined"
              :rules="[(v) => !!v || 'Role is required']"
            ></v-select>

            <v-row class="align-center justify-space-between">
              <v-col cols="6">
                <v-checkbox
                  v-model="remember"
                  label="Remember me"
                  hide-details
                ></v-checkbox>
              </v-col>
              <v-col cols="6" class="text-right">
                <a
                  href="#"
                  style="text-decoration: none; font-size: 12px; color: #1976d2"
                >
                  Forgot password?
                </a>
              </v-col>
            </v-row>

            <v-btn color="#1976D2" block class="mt-4" type="submit" :loading="loading">
              <v-icon left>mdi-arrow-right</v-icon> Sign In
            </v-btn>
          </v-form>

          <div class="text-center mt-4">
            <small>
              <a href="#" style="text-decoration: none; color: grey">Privacy Policy</a>
              |
              <a href="#" style="text-decoration: none; color: grey">Terms of Service</a>
              |
              <a href="#" style="text-decoration: none; color: grey">Support</a>
            </small>
            <br />
          </div>
        </v-card>
      </v-container>
    </v-main>
  </v-app>
</template>

<script>
import AdminHeader from "@/components/AdminHeader.vue";
import { ref } from "vue";
import { useRouter } from "vue-router";

export default {
  setup() {
    const router = useRouter();

    const username = ref("");
    const password = ref("");
    const role = ref(null);
    const roles = ["Administrative", "Engineer", "Architect", "Inspector"];
    const show = ref(false);
    const valid = ref(false);
    const error = ref("");
    const loading = ref(false);
    const remember = ref(false);

    const login = async () => {
      error.value = "";

      // Optional: Manual validation fallback
      if (!username.value || !password.value || !role.value) {
        error.value = "Please fill in all fields.";
        return;
      }

      loading.value = true;

      // Fake authentication simulation
      setTimeout(() => {
        loading.value = false;

        const isValid =
          (username.value === "admin" &&
            password.value === "1234" &&
            role.value === "Administrative") ||
          (username.value === "engineer" &&
            password.value === "pass123" &&
            role.value === "Engineer") ||
          (username.value === "architect" &&
            password.value === "pass123" &&
            role.value === "Architect");
        username.value === "inspector" &&
          password.value === "pass123" &&
          role.value === "Inspector";

        if (isValid) {
          alert(`✅ Login successful as ${role.value}!`);

          // Redirection logic based on role
          if (role.value === "Administrative") {
            router.push("/");
          } else if (role.value === "Architect") {
            router.push("/");
          } else if (role.value === "Engineer") {
            router.push("/");
          } else if (role.value === "Inspector") {
            router.push("/");
          }
        } else {
          error.value = "Invalid credentials or role.";
        }
      }, 1000);
    };

    return {
      username,
      password,
      role,
      roles,
      show,
      login,
      valid,
      error,
      loading,
      remember,
    };
  },
};
</script>

<style scoped>
.nav-links .v-btn {
  text-transform: none !important;
  font-weight: 500;
  font-size: 17px;
}
</style>
